...
def test_unpack_binary_restoration_entry() -> None:
    ...
    assert result.spend_transaction_hash == SPEND_TRANSACTION_HASH
    assert result.transaction_output_index == TRANSACTION_OUTPUT_INDEX
    assert result.spend_input_index == SPEND_INPUT_INDEX
